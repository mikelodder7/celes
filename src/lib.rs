...
fn lookup_ascii_lowercase<'a, V>(map: &'a Map<&'static str, V>, key: &str) -> Option<&'a V> {
    ...
    if key.len() <= buf.len() {
        ...
        let lowered = core::str::from_utf8(buf).ok()?;
        map.get(lowered)
    } else {
        map.get(key)
    }
...
impl Country {
    ...
    country!(czechia, "203", 203, "CZ", "CZE", "Czechia", "CzechRepublic");
    ...
    country!(macao, "446", 446, "MO", "MAC", "Macao", "Macau");
    ...
    country!(
        myanmar, "104", 104, "MM", "MMR", "Myanmar", "Myanmar", "Burma"
    );
    ...
    country!(the_gambia, "270", 270, "GM", "GMB", "The Gambia", "Gambia");
    ...
    country!(
        turkiye, "792", 792, "TR", "TUR", "Türkiye", "Turkiye", "Turkey"
    );
    ...
    /// Returns whether this country has `alias`, using ASCII case-insensitive matching.
    #[must_use]
    ...
    /// ```
    pub fn from_value(value: usize) -> Result<Self, CountryParseError> {
        VALUE_MAP
            .get(&value)
            .copied()
            .map(|make_country| make_country())
    ...
    /// ```
    pub fn from_code<A: AsRef<str>>(code: A) -> Result<Self, CountryParseError> {
        NUMERIC_CODE_MAP
            .get(code.as_ref())
            .copied()
            .map(|make_country| make_country())
    ...
    /// assert_eq!(Country::the_united_states_of_america(), res.unwrap());
    /// ```
    pub fn from_alpha2<A: AsRef<str>>(alpha2: A) -> Result<Self, CountryParseError> {
        lookup_ascii_lowercase(&ALPHA2_MAP, alpha2.as_ref())
            .copied()
            .map(|make_country| make_country())
    ...
    /// assert_eq!(Country::the_united_states_of_america(), res.unwrap());
    /// ```
    pub fn from_alpha3<A: AsRef<str>>(alpha3: A) -> Result<Self, CountryParseError> {
        lookup_ascii_lowercase(&ALPHA3_MAP, alpha3.as_ref())
            .copied()
            .map(|make_country| make_country())
    ...
    /// ```
    pub fn from_alias<A: AsRef<str>>(alias: A) -> Result<Self, CountryParseError> {
        lookup_ascii_lowercase(&ALIAS_MAP, alias.as_ref())
            .copied()
            .map(|make_country| make_country())
    ...
    /// assert_eq!(Country::the_united_kingdom_of_great_britain_and_northern_ireland(), res.unwrap());
    /// ```
    pub fn from_name<A: AsRef<str>>(name: A) -> Result<Self, CountryParseError> {
        lookup_ascii_lowercase(&NAME_MAP, name.as_ref())
            .copied()
            .map(|make_country| make_country())
            .ok_or(CountryParseError::InvalidName)
    }
}
...
impl FromStr for Country {
    ...
    fn from_str(code: &str) -> Result<Self, Self::Err> {
        lookup_ascii_lowercase(&PARSE_MAP, code)
            .copied()
            .map(|make_country| make_country())
            .ok_or(CountryParseError::UnknownIdentifier)
    }
}

static COUNTRIES: [Country; 250] = [
    Country::afghanistan(),
    ...
    Country::djibouti(),
    Country::dominica(),
    Country::dutch_part_sint_maarten(),
    Country::ecuador(),
    Country::egypt(),
    Country::el_salvador(),
    ...
    Country::french_part_saint_martin(),
    ...
    Country::iraq(),
    Country::ireland(),
    Country::islamic_republic_of_iran(),
    Country::isle_of_man(),
    Country::israel(),
    Country::italy(),
    Country::jamaica(),
...
static VALUE_MAP: Map<usize, CountryConstructor> = phf_map! {
...
static NUMERIC_CODE_MAP: Map<&'static str, CountryConstructor> = phf_map! {
...
static ALPHA2_MAP: Map<&'static str, CountryConstructor> = phf_map! {
...
static ALPHA3_MAP: Map<&'static str, CountryConstructor> = phf_map! {
...
static ALIAS_MAP: Map<&'static str, CountryConstructor> = phf_map! {
    ...
    "台灣" => Country::taiwan,
    "republic of china" => Country::taiwan,
    "中華民國" => Country::taiwan,
...
static NAME_MAP: Map<&'static str, CountryConstructor> = phf_map! {
...
static PARSE_MAP: Map<&'static str, CountryConstructor> = phf_map! {
    "afghanistan" => Country::afghanistan,
    "004" => Country::afghanistan,
    "af" => Country::afghanistan,
    "afg" => Country::afghanistan,
